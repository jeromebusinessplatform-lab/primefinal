...
function manilaOrderNumber(value: number | Date) {
...
function primeMemberId(telegramId: string) { return `PRIME-${telegramId.slice(-8).padStart(8, "0")}`; }
...
export function installReleaseRoutes(app: Application) {
  ...
  app.get("/api/orders", async (req, res) => {
    ...
    try {
      ...
      const orders = snap.docs.map(d => {
        const raw = plain(d.id, d.data());
        if (!raw.orderNumber) raw.orderNumber = manilaOrderNumber(Number(raw.createdAt || Date.now()));
        return raw;
  ...
  app.post("/api/orders", async (req, res) => {
    ...
    const firestore = db(); const orderRef = firestore.collection("orders").doc(); const customerRef = firestore.collection("customers").doc(tg); const submittedAt = new Date(); const orderNumber = manilaOrderNumber(submittedAt);
    try {
      const result = await firestore.runTransaction(async tx => {
        ...
        const order = { ...input, telegramUserId: tg, items: normalized, subtotal, discount, charges, deliveryFee, total, orderNumber, paymentStatus: "PENDING", orderStatus: "REVIEW", createdAt: now, updatedAt: now };
        tx.set(orderRef, order); tx.set(customerRef, { id: tg, telegramUserId: tg, telegramDisplayName: String(input.telegramDisplayName || "Unknown"), telegramUsername: input.telegramUsername || null, primeMemberId: primeMemberId(tg), vipTier: "Bronze", updatedAt: now }, { merge: true }); return { ...order, id: orderRef.id, createdAt: submittedAt.getTime() };
      }); return res.status(201).json({ order: result });
    } catch (e: any) { return res.status(400).json({ error: e?.message || "Unable to create order" }); }
  });

  app.post("/api/customer/migration", async (req, res) => {
    const tg = telegramUserId(req); if (!tg) return res.status(401).json({ error: "Authentication required" });
    try {
      const ref = db().collection("customers").doc(tg); const snap = await ref.get();
      if (!snap.exists) return res.status(404).json({ error: "Customer account not found" });
      const current = String(snap.data()?.primeMemberId || "");
      if (!current || current.startsWith("PC")) {
        const migrated = primeMemberId(tg); await ref.set({ primeMemberId: migrated, updatedAt: FieldValue.serverTimestamp(), primeMemberIdMigratedAt: FieldValue.serverTimestamp(), primeMemberIdPrevious: current || null }, { merge: true });
        return res.json({ migrated: true, previousId: current || null, primeMemberId: migrated, notification: `Your PRIME™ Member ID has been migrated from ${current || "the legacy ID"} to ${migrated}.` });
      }
      return res.json({ migrated: false, primeMemberId: current });
    } catch { return res.status(500).json({ error: "Unable to migrate PRIME™ Member ID" }); }
  });
  ...
  app.get("/api/customers", async (req, res) => {
    ...
    try {
      ...
      if (isAdmin) { const snap = await ref.orderBy("updatedAt", "desc").get(); return res.json({ customers: snap.docs.map(d => plain(d.id, d.data())) }); }
      const snap = await ref.doc(tg).get(); return res.json({ customers: snap.exists ? [plain(snap.id, snap.data())] : [] });
