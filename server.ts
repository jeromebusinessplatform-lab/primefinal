    ...
    if (apiKey && apiKey.trim().length > 0) {
      ...
      if (response.ok) {
        ...
        if (data.features?.length > 0) {
          ...
          return res.json({ formatted: f.properties.formatted, lat: f.properties.lat, lon: f.properties.lon, city: f.properties.city, country: f.properties.country, source: "geoapify" });
        }
      }
    }
...
app.get("/api/geo/route", async (req, res) => {
  try {
    ...
    if (apiKey && apiKey.trim().length > 0) {
      ...
      if (response.ok) {
        ...
        if (data.features?.length > 0) {
          ...
          return res.json({ distanceKm: Number(((feature.properties.distance || 0) / 1000).toFixed(2)), durationMinutes: Math.ceil((feature.properties.time || 0) / 60), coordinates: feature.geometry?.coordinates || [], legs: feature.properties.legs || [], source: "geoapify" });
        }
      }
    }
    ...
    for (let i = 0; i <= 6; i++) {
      const t = i / 6;
      coordinates.push([fromLon + (toLon - fromLon) * t + Math.sin(t * Math.PI) * 0.005, fromLat + (toLat - fromLat) * t + Math.sin(t * Math.PI) * 0.003]);
    }
...
function analyzeReceiptHeuristic(rawImageString: string, expectedAmount?: number, expectedReceiver?: string) {
  ...
  if (/maya/i.test(decoded)) { channel = "Maya"; referenceNumber = "MYA-" + Math.floor(1000 + Math.random() * 9000) + "-" + Math.floor(1000 + Math.random() * 9000); confidenceScore = 96; }
  else if (/bpi/i.test(decoded)) { channel = "BPI"; channelType = "BANK_TRANSFER"; referenceNumber = "BPI-FT-" + now.toISOString().slice(0, 10).replace(/-/g, "") + "-" + Math.floor(1000 + Math.random() * 9000); confidenceScore = 95; }
  else if (/bdo/i.test(decoded)) { channel = "BDO"; channelType = "BANK_TRANSFER"; referenceNumber = "BDO-REF-" + Math.floor(1000000000 + Math.random() * 9000000000); confidenceScore = 93; }
  else if (/pos|official receipt|invoice/i.test(decoded)) { channel = "Store POS Invoice"; channelType = "PHYSICAL_RECEIPT"; referenceNumber = "OR# " + now.getFullYear() + "-" + Math.floor(10000 + Math.random() * 90000); confidenceScore = 92; }
  const amountMatch = decoded.match(/(?:PHP|₱|\$)\s*([\d,]+(?:\.\d{2})?)/i);
  if (amountMatch) { const parsedAmt = parseFloat(amountMatch[1].replace(/,/g, "")); if (!isNaN(parsedAmt) && parsedAmt > 0) amount = parsedAmt; }
...
app.get("/api/ocr/status", (_req, res) => {
  ...
  res.json({ enabled: true, model: "gemini-3.7-flash", hasApiKey: hasGeminiKey, supportedChannels: ["GCash", "Maya", "BPI Mobile", "BDO Digital", "UnionBank", "Metrobank", "InstaPay", "PESONet", "Physical POS Receipts"] });
});
  ...
  try {
    ...
    if (taggunApiKey && taggunApiKey.trim().length > 0) {
      try {
        ...
        const response = await axios.post("https://api.taggun.io/api/receipt/v1/verbose/file", formData, { headers: { ...formData.getHeaders(), apikey: taggunApiKey } });
        ...
        return res.json({ success: true, result: { ...analyzeReceiptHeuristic(imageBase64, expectedAmount, expectedReceiver), executionTimeMs: Date.now() - startTime }, warning: "Processed via resilient fallback engine" });
      }
    }
    ...
    return res.status(500).json({ success: false, error: error.message || "Internal server error during OCR receipt processing" });
  }
});
