...
function Field({ label, icon, children }: { label: string; icon: React.ReactNode; children: React.ReactNode }) { return <label className="block"><span className="text-[11px] text-neutral-600 uppercase flex items-center gap-1" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>{icon}{label}</span><div className="mt-1">{children}</div></label>; }
...
function StepButton({ label }: { label: string }) { return <button type="submit" className="flex-[2] bg-black text-white py-3.5 rounded-xl flex items-center justify-center gap-2 shadow-md" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "15px" }}>{label}<ArrowRight size={17} /></button>; }
