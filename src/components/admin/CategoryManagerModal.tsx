    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-md border border-neutral-200 shadow-2xl p-5 space-y-4 max-h-[85vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-neutral-100 pb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-neutral-500 font-normal" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>Add, rename, or delete catalog product categories</p>
            </div>
          </div>
        ...
        <form onSubmit={handleAdd} className="flex gap-2">
          <input type="text" placeholder="New category name (e.g. Smart Home)" value={newCatName} onChange={(e) => setNewCatName(e.target.value)} disabled={saving} className="flex-1 bg-neutral-50 border border-neutral-300 rounded-xl px-3 py-2 text-xs outline-none focus:border-black font-normal" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "14px" }} />
          <button type="submit" disabled={!newCatName.trim() || saving} className="bg-black hover:bg-neutral-800 disabled:bg-neutral-300 text-white px-3.5 py-2 rounded-xl text-xs font-normal flex items-center gap-1 cursor-pointer transition-colors shadow-2xs" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}><Plus size={14} /> Add</button>
        </form>
            ...
            return (
              <div key={cat} className="pt-2 first:pt-0 flex items-center justify-between gap-2 group">
                {isEditing ? (
                  ...
                  <>
                    <div className="flex items-center gap-2 min-w-0"><span className="text-sm font-medium text-neutral-900 truncate" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "15px" }}>{cat}</span><span className="text-[10px] bg-neutral-100 text-neutral-600 font-mono px-1.5 py-0.2 rounded-full">{count} item{count === 1 ? "" : "s"}</span></div>
        ...
        <div className="pt-3 border-t border-neutral-100 flex justify-end"><button type="button" onClick={onClose} disabled={saving} className="bg-neutral-100 hover:bg-neutral-200 text-neutral-800 px-4 py-1.5 rounded-xl text-xs font-normal cursor-pointer" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>Done</button></div>
      </div>
    </div>
  );
}
